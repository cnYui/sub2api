...
interface NavItem {
  path: string
  /** 保持激活态和菜单点击逻辑使用稳定路径，实际跳转可附带嵌入式页面参数。 */
  to?: RouteLocationRaw
  label: string
  icon: unknown
  iconSvg?: string
...
//
// 条目顺序：密钥 → 用量 → 模型广场 → 可用渠道 → 渠道状态 → 订阅/支付 → 兑换 → 使用方法 → 资料。
// 模型广场紧挨可用渠道，让用户先查看全量价格，再按自己的权限核对可用渠道。
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/redeem-cards', label: t('nav.redeemCards'), icon: RedeemCardIcon, hideInSimpleMode: true },
