    ...
    <template #footer>
      <div class="flex justify-end space-x-3">
        ...
        <button
          ...
          :disabled="submitting"
          :class="[
            'rounded-md px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-dark-800',
            danger
              ? 'bg-red-600 hover:bg-red-700 focus:ring-red-500'
              : 'bg-primary-600 hover:bg-primary-700 focus:ring-primary-500'
          ]"
        >
          {{ submitting ? t('common.loading') : confirmText }}
        </button>
      </div>
    </template>
  </BaseDialog>
