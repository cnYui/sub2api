    ...
    <div class="flex flex-1 flex-col p-4">
      ...
      <div class="mb-3 flex items-start justify-between gap-2">
        <div class="min-w-0 flex-1">
          ...
          <p v-if="displayDescription" class="mt-0.5 text-xs leading-relaxed text-gray-500 dark:text-dark-400 line-clamp-2">
            {{ displayDescription }}
          </p>
        </div>
      ...
      <div v-if="!isSimplifiedMonthlyPlan" class="mb-3 grid grid-cols-2 gap-x-3 gap-y-1 rounded-lg bg-gray-50 px-3 py-2 text-xs dark:bg-dark-700/50">
      ...
      <div v-if="!isSimplifiedMonthlyPlan && plan.features.length > 0" class="mb-3 space-y-1">
...
const isSimplifiedMonthlyPlan = computed(() => props.plan.daily_limit_usd != null && props.plan.validity_days > 0)
...
const simplifiedSummary = computed(() => {
  const dailyLimit = props.plan.daily_limit_usd == null ? '0' : formatCompactNumber(props.plan.daily_limit_usd)
  return `月度订阅-时间 ${props.plan.validity_days}天，日限额 ${dailyLimit}刀，24点刷新`
})
const displayDescription = computed(() => (isSimplifiedMonthlyPlan.value ? simplifiedSummary.value : props.plan.description))
