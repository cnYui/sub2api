export default {
    ...
    channelMonitor: {
      ...
      syncGroups: '从分组同步',
      syncGroupsHint: '按启用分组和账号白名单生成监控，直接用账号的上游地址和 Key 探测 /v1/models。系统每 10 分钟也会自动同步一次。',
      syncGroupsSuccess: '同步完成：新增 {created}，更新 {updated}，停用 {disabled}，未变 {unchanged}',
      syncGroupsSkipped: '有 {n} 个账号没有生成监控：{detail}',
      syncGroupsFailed: '从分组同步失败',
      groupSyncedBadge: '分组同步',
      groupSyncedTooltip: '由分组同步维护（来源分组 #{group}、账号 #{account}）。名称、地址、Key 和模型会在下次同步时按分组与账号白名单覆盖；主模型只要仍在白名单里就保留。',
      groupSyncedEditNotice: '这条监控由分组同步维护：名称、上游地址、Key、模型清单和启用状态会在下次同步时被覆盖。要改模型请去改账号白名单；这里只有主模型、检测间隔和抖动的修改会保留。',
      ...
      form: {
        ...
        provider: '平台',
        apiMode: '监控方式',
        apiModeModels: '目录探测（GET /v1/models）',
        apiModeModelsHint: '只检查鉴权、HTTP 2xx 和目标模型是否在目录中，不发送真实推理请求，不消耗推理 token。',
    ...
    subscriptions: {
      ...
      selectUser: '选择用户',
      selectGroup: '选择订阅分组',
      selectBalancePackage: '选择余额套餐',
      groupHint: '仅显示订阅计费类型的分组',
      balancePackageHint: '仅显示购买页当前可售的套餐，发放后立即到账首期余额。',
      ...
      failedToGrantBalancePackage: '发放套餐失败',
      ...
      pleaseSelectBalancePackage: '请选择余额套餐',
