export default {
  ...
  userSubscriptions: {
    ...
    earlyRefresh: '提前刷新',
    earlyRefreshTitle: '提前刷新下一期额度',
    earlyRefreshMessage: '将立即发放第 {next} / {total} 期额度 ${amount}，并把下次自动刷新时间顺延一个周期。套餐到期时间不变。',
    earlyRefreshNote: '套餐总额度仍是 {total} 期，提前刷新只是把后面的期数挪到现在，不会多给也不会少给。',
    earlyRefreshConfirm: '确认刷新',
    earlyRefreshBlockedQuota: '本周还有 ${amount} 未用完，用完后即可提前刷新下一期。',
    earlyRefreshSuccess: '已发放第 {count} / {total} 期额度 ${amount}',
    earlyRefreshFailed: '提前刷新失败，请稍后重试',
    buyAgain: '再次购买',
  ...
  payment: {
    ...
    refundQuote: {
      ...
      retainedQuota: '其中取消套餐时留在余额的额度',
      reclaimQuota: '退款后从余额扣回',
      reclaimNotice: '退款成功后，本周尚未用完的 {amount} 额度会从余额中扣除。',
      usageRatio: '额度使用比例',
      timeRatio: '时间使用比例',
    ...
    admin: {
      ...
      refundOrder: '退款订单',
      refundAmount: '退款金额',
      lastAttemptedRefund: '上次尝试的退款金额',
      refundQuoteLive: '金额按当前时间和用量实时计算，确认时服务端会再算一次，最终金额可能略低。',
      refundQuoteFailed: '无法计算退款金额，暂时不能退款。',
      refundReclaimNotice: '退款成功后，会从用户余额里扣回本周尚未用完的 {amount} 额度。',
