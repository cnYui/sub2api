...
describe('PlanEditDialog', () => {
  it('保存公共 Codex 套餐时固定提交 28 天有效期', async () => {
    ...
    expect((textareas[0].element as HTMLTextAreaElement).value).toContain('28 天订阅，每 7 天刷新 198 USD 周额度，购买时间起滚动计算')
    expect((textareas[0].element as HTMLTextAreaElement).value).not.toContain('历史漂移套餐')
    expect((textareas[1].element as HTMLTextAreaElement).value).toContain('周额度 198 USD')
    expect(wrapper.text()).toContain('payment.admin.publicCodex28DayHint')
    expect(wrapper.text()).toContain('$198')
    expect(wrapper.text()).toContain('$792')
