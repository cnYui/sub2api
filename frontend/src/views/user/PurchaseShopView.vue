...
import type { BalancePackagePlan, CheckoutInfoResponse, CreateOrderResult, OrderType } from '@/types/payment'

interface ProductDetail { label: string; value: string }
interface CatalogProduct { id: string; product: PurchaseProductCardModel; balancePackage: BalancePackagePlan }
...
const viewMode = ref<'catalog' | 'balance_package' | 'paying'>('catalog')
const selectedBalancePackage = ref<BalancePackagePlan | null>(null)
const selectedMethod = ref('')
const paymentState = ref<PaymentRecoverySnapshot>(emptyPaymentState())
const checkout = ref<CheckoutInfoResponse>({
  methods: {}, global_min: 0, global_max: 0, plans: [], balance_packages: [], balance_disabled: false,
...
const currentAmount = computed(() => selectedBalancePackage.value?.price_cny || 0)
...
async function submitOrder(): Promise<void> {
  ...
  const orderType: OrderType = 'balance_subscription'
  submitting.value = true
  errorMessage.value = ''
  try {
    const result = await paymentStore.createOrder(buildCreateOrderPayload({
      amount: currentAmount.value, paymentType: selectedMethod.value, orderType, balancePackagePlanId: selectedBalancePackage.value?.id,
