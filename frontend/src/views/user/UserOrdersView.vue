        ...
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-800">
          ...
          <template v-if="refundQuote">
            <div class="mt-2 border-t border-gray-200 pt-2 text-sm dark:border-dark-700">
              ...
              <div v-if="!refundQuote.manual_review_required" class="mt-1 flex justify-between"><span class="text-gray-500 dark:text-gray-400">{{ t('payment.refundQuote.timeRatio') }}</span><span>{{ Math.round(refundQuote.time_ratio * 100) }}%</span></div>
              <div v-if="!refundQuote.manual_review_required" class="mt-1 flex justify-between"><span class="text-gray-500 dark:text-gray-400">{{ t('payment.refundQuote.consumptionRatio') }}</span><span>{{ Math.round(refundQuote.consumption_ratio * 100) }}%</span></div>
