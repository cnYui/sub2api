...
func TestOpenAIImagesBudgetBody_ExcludesMultipartUploadBytes(t *testing.T) {
	...
	writer := multipart.NewWriter(&body)
	require.NoError(t, writer.WriteField("model", "gpt-image-2"))
	require.NoError(t, writer.WriteField("prompt", "replace background with a blue sky"))
	require.NoError(t, writer.WriteField("size", "1536x1024"))
	part, err := writer.CreateFormFile("image", "source.png")
	require.NoError(t, err)
	_, err = part.Write(bytes.Repeat([]byte("x"), 3<<20))
	require.NoError(t, err)
	require.NoError(t, writer.Close())
	...
	require.Less(t, len(budgetBody), 1_024)
...
func TestOpenAIGatewayServiceForwardImages_APIKeyNonStreamOutputTokensBillAsImageWhenDetailsMissing(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","response_format":"b64_json"}`)
	...
	svc := &OpenAIGatewayService{
		...
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"application/json"},
					"X-Request-Id": []string{"req_img_no_output_details"},
				},
				Body: io.NopCloser(strings.NewReader(`{
					"created":1710000011,
					"usage":{
						...
						"output_tokens":2058,
						"total_tokens":3155,
						"input_tokens_details":{"image_tokens":992,"text_tokens":105}
					},
	...
	require.NotNil(t, result)
	require.Equal(t, 1, result.ImageCount)
	require.Equal(t, 992, result.Usage.ImageInputTokens)
	require.Equal(t, 2058, result.Usage.ImageOutputTokens)
}
