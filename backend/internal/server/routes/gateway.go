...
func RegisterGatewayRoutes(
	...
	resolveGroupAnthropic := middleware.ResolveEffectiveGroupForSupportedEndpoints(effectiveGroupResolver, middleware.AnthropicErrorWriter)
	resolveGroupGoogle := middleware.ResolveEffectiveGroupForSupportedEndpoints(effectiveGroupResolver, middleware.GoogleErrorWriter)
	...
	antigravityV1.Use(resolveGroupAnthropic)
	...
	antigravityV1Beta.Use(resolveGroupGoogle)
...
func openAIOnly(featureName string, handler gin.HandlerFunc) gin.HandlerFunc {
	return func(c *gin.Context) {
		if getGroupPlatform(c) != service.PlatformOpenAI {
			...
			c.JSON(http.StatusNotFound, gin.H{
				"error": gin.H{
					"type":    "not_found_error",
					"message": featureName + " API is not supported for this platform",
				},
			})
			return
		}
