...
func TestPaymentHandlerGetPlansIncludesGroupLimits(t *testing.T) {
	...
	require.Equal(t, "28 天订阅，每 7 天刷新 58 USD 周额度，购买时间起滚动计算", resp.Data[0].Description)
	...
	require.InDelta(t, 58, *resp.Data[0].WeeklyLimitUSD, 0.0000001)
	require.Nil(t, resp.Data[0].MonthlyLimitUSD)
	require.NotNil(t, resp.Data[0].PeriodTotalQuotaUSD)
	require.InDelta(t, 232, *resp.Data[0].PeriodTotalQuotaUSD, 0.0000001)
...
func TestPaymentHandlerGetCheckoutInfoUsesPublicCodexQuotaSnapshot(t *testing.T) {
	...
	require.Equal(t, "28 天订阅，每 7 天刷新 400 USD 周额度，购买时间起滚动计算", plan.Description)
	require.Equal(t, "199 元订阅池", plan.ProductName)
	require.Equal(t, []string{"周额度 400 USD", "28 天有效期", "购买时间起每 7 天刷新"}, plan.Features)
	require.Nil(t, plan.DailyLimitUSD)
	require.NotNil(t, plan.WeeklyLimitUSD)
	require.InDelta(t, 400, *plan.WeeklyLimitUSD, 0.0000001)
	require.Nil(t, plan.MonthlyLimitUSD)
	require.NotNil(t, plan.PeriodTotalQuotaUSD)
	require.InDelta(t, 1600, *plan.PeriodTotalQuotaUSD, 0.0000001)
