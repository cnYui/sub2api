...
// inboundIsResponses 判断当前请求是否落在正式 Responses API 路由上。
//
// 不能直接用 GetInboundEndpoint(c) == EndpointResponses 比较，因为
// 测试和部分 handler 直连会使用 /openai/v1/responses。裸 /responses 与
// /backend-api/codex/responses 已由 INVALID_BASE_URL 处理，不再属于有效 API。
func inboundIsResponses(c *gin.Context) bool {
	...
	return endpointPathMatches(p, EndpointResponses, true) ||
		endpointPathMatches(p, "/openai"+EndpointResponses, true)
}
