...
type AdminPaymentOrderResult struct {
	...
	RefundBasis          map[string]any `json:"refund_basis,omitempty"`
...
// GetRefundQuote 返回管理员只读退款报价；普通退款提交时仍会在事务内重新计算。
...
func (h *PaymentHandler) GetRefundQuote(c *gin.Context) {
	...
	quote, err := h.paymentService.AdminGetSubscriptionRefundQuote(c.Request.Context(), orderID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *PaymentHandler) ListPlans(c *gin.Context) {
	...
	service.NormalizeSubscriptionPlansForDisplay(plans, groupInfo)
...
func (h *PaymentHandler) CreatePlan(c *gin.Context) {
	...
	if plan != nil {
		...
		service.NormalizeSubscriptionPlansForDisplay([]*dbent.SubscriptionPlan{plan}, groupInfo)
	}
