...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	if err != nil {
		reqLog.Warn("gateway.responses.api_key_slot_acquire_failed", zap.Error(err))
	...
	// 2. Re-check billing
	if failure := checkGatewayBillingEligibility(requestCtx, c, h.billingCacheService, apiKey, subscription); failure != nil {
		reqLog.Info("gateway.responses.billing_check_failed", zap.Error(failure.Err))
		h.responsesErrorResponse(c, failure.Status, failure.Code, failure.Message)
		return
	}
	...
	for {
		...
		if err := finalizeUsageFactResponse(c, usageGate, func(ctx context.Context) error {
			...
			reqLog.Error("gateway.responses.persist_usage_fact_failed",
				zap.Int64("account_id", account.ID),
				zap.Error(err),
			)
			return
