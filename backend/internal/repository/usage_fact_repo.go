...
func (r *usageFactRepository) CreatePending(ctx context.Context, fact *service.UsageFact) (*service.UsageFact, bool, error) {
	...
	created, err := scanUsageFact(r.db.QueryRowContext(ctx, `
			INSERT INTO usage_facts (
				...
				payload_version, payload, billing_status, next_attempt_at, completed_at, authorization_id, entitlement_period_id
			)
			VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending', $8, $9, $10, $11)
			ON CONFLICT (request_id, api_key_id) DO NOTHING
			RETURNING id, request_id, api_key_id, user_id, account_id,
				request_fingerprint, authorization_id, entitlement_period_id, payload_version, payload, billing_status,
	...
	rows, err := r.db.QueryContext(ctx, `
		...
		FROM candidates
			...
			RETURNING f.id, f.request_id, f.api_key_id, f.user_id, f.account_id,
				f.request_fingerprint, f.authorization_id, f.entitlement_period_id, f.payload_version, f.payload, f.billing_status,
...
func scanUsageFact(scanner usageFactScanner) (*service.UsageFact, error) {
	...
	if fact.AuthorizationID != nil {
		// 保留内存兼容字段，避免未迁移的结算调用方丢失同一条授权关联。
		fact.ReservationID = fact.AuthorizationID
	}
	return &fact, nil
}
