...
BEGIN
    ...
    SELECT
        ...
        NOW()
    WHERE NOT EXISTS (
        SELECT 1 FROM groups WHERE name = 'codex-pool-69-usd' AND deleted_at IS NULL
    )
    RETURNING id INTO target_group_id;

    SELECT id INTO target_group_id
    FROM groups
    WHERE name = 'codex-pool-69-usd' AND deleted_at IS NULL
    ORDER BY id
    LIMIT 1;

    UPDATE groups
    SET
        description = '79 元订阅池，每日 69 USD，30 天有效期',
        rate_multiplier = 1.0,
        is_exclusive = FALSE,
        status = 'active',
        platform = 'openai',
    ...
    SET
        ...
        validity_unit = 'day',
        features = '',
        product_name = '79 元订阅池',
        for_sale = TRUE,
        sort_order = 79,
        updated_at = NOW()
    WHERE group_id = target_group_id
       OR name = '79 元订阅池'
       OR product_name = '79 元订阅池';
