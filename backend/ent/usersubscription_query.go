...
type UserSubscriptionQuery struct {
	...
	withEntitlementPeriods   *SubscriptionEntitlementPeriodQuery
	withQuotaDebtAdjustments *SubscriptionQuotaDebtAdjustmentQuery
...
// QueryEntitlementPeriods chains the current query on the "entitlement_periods" edge.
func (_q *UserSubscriptionQuery) QueryEntitlementPeriods() *SubscriptionEntitlementPeriodQuery {
	query := (&SubscriptionEntitlementPeriodClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryQuotaDebtAdjustments chains the current query on the "quota_debt_adjustments" edge.
func (_q *UserSubscriptionQuery) QueryQuotaDebtAdjustments() *SubscriptionQuotaDebtAdjustmentQuery {
...
func (_q *UserSubscriptionQuery) Clone() *UserSubscriptionQuery {
	...
	return &UserSubscriptionQuery{
		...
		withEntitlementPeriods:   _q.withEntitlementPeriods.Clone(),
		withQuotaDebtAdjustments: _q.withQuotaDebtAdjustments.Clone(),
...
// WithEntitlementPeriods tells the query-builder to eager-load the nodes that are connected to
// the "entitlement_periods" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserSubscriptionQuery) WithEntitlementPeriods(opts ...func(*SubscriptionEntitlementPeriodQuery)) *UserSubscriptionQuery {
	query := (&SubscriptionEntitlementPeriodClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withEntitlementPeriods = query
	return _q
}

// WithQuotaDebtAdjustments tells the query-builder to eager-load the nodes that are connected to
// the "quota_debt_adjustments" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserSubscriptionQuery) WithQuotaDebtAdjustments(opts ...func(*SubscriptionQuotaDebtAdjustmentQuery)) *UserSubscriptionQuery {
	query := (&SubscriptionQuotaDebtAdjustmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withQuotaDebtAdjustments = query
	return _q
}
...
func (_q *UserSubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserSubscription, error) {
	var (
		...
		loadedTypes = [6]bool{
			...
			_q.withEntitlementPeriods != nil,
			_q.withQuotaDebtAdjustments != nil,
		}
	)
	...
	if query := _q.withEntitlementPeriods; query != nil {
		if err := _q.loadEntitlementPeriods(ctx, query, nodes,
			func(n *UserSubscription) { n.Edges.EntitlementPeriods = []*SubscriptionEntitlementPeriod{} },
			func(n *UserSubscription, e *SubscriptionEntitlementPeriod) {
				n.Edges.EntitlementPeriods = append(n.Edges.EntitlementPeriods, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := _q.withQuotaDebtAdjustments; query != nil {
		if err := _q.loadQuotaDebtAdjustments(ctx, query, nodes,
			func(n *UserSubscription) { n.Edges.QuotaDebtAdjustments = []*SubscriptionQuotaDebtAdjustment{} },
			func(n *UserSubscription, e *SubscriptionQuotaDebtAdjustment) {
				n.Edges.QuotaDebtAdjustments = append(n.Edges.QuotaDebtAdjustments, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UserSubscriptionQuery) loadEntitlementPeriods(ctx context.Context, query *SubscriptionEntitlementPeriodQuery, nodes []*UserSubscription, init func(*UserSubscription), assign func(*UserSubscription, *SubscriptionEntitlementPeriod)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*UserSubscription)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptionentitlementperiod.FieldSubscriptionID)
	}
	query.Where(predicate.SubscriptionEntitlementPeriod(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usersubscription.EntitlementPeriodsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserSubscriptionQuery) loadQuotaDebtAdjustments(ctx context.Context, query *SubscriptionQuotaDebtAdjustmentQuery, nodes []*UserSubscription, init func(*UserSubscription), assign func(*UserSubscription, *SubscriptionQuotaDebtAdjustment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*UserSubscription)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptionquotadebtadjustment.FieldSubscriptionID)
	}
	query.Where(predicate.SubscriptionQuotaDebtAdjustment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usersubscription.QuotaDebtAdjustmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
