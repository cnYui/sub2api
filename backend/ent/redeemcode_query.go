...
type RedeemCodeQuery struct {
	...
	withBalancePackagePlan *BalancePackagePlanQuery
...
// QueryBalancePackagePlan chains the current query on the "balance_package_plan" edge.
func (_q *RedeemCodeQuery) QueryBalancePackagePlan() *BalancePackagePlanQuery {
	query := (&BalancePackagePlanClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *RedeemCodeQuery) Clone() *RedeemCodeQuery {
	...
	return &RedeemCodeQuery{
		...
		withBalancePackagePlan: _q.withBalancePackagePlan.Clone(),
...
// WithBalancePackagePlan tells the query-builder to eager-load the nodes that are connected to
// the "balance_package_plan" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RedeemCodeQuery) WithBalancePackagePlan(opts ...func(*BalancePackagePlanQuery)) *RedeemCodeQuery {
	query := (&BalancePackagePlanClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withBalancePackagePlan = query
	return _q
}
...
func (_q *RedeemCodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RedeemCode, error) {
	...
	if query := _q.withBalancePackagePlan; query != nil {
		if err := _q.loadBalancePackagePlan(ctx, query, nodes, nil,
			func(n *RedeemCode, e *BalancePackagePlan) { n.Edges.BalancePackagePlan = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RedeemCodeQuery) loadBalancePackagePlan(ctx context.Context, query *BalancePackagePlanQuery, nodes []*RedeemCode, init func(*RedeemCode), assign func(*RedeemCode, *BalancePackagePlan)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*RedeemCode)
	for i := range nodes {
		if nodes[i].BalancePackagePlanID == nil {
			continue
		}
		fk := *nodes[i].BalancePackagePlanID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(balancepackageplan.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "balance_package_plan_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RedeemCodeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withBalancePackagePlan != nil {
			_spec.Node.AddColumnOnce(redeemcode.FieldBalancePackagePlanID)
		}
	}
