...
// QuerySubscriptionEntitlementPeriods chains the current query on the "subscription_entitlement_periods" edge.
func (_q *UserQuery) QuerySubscriptionEntitlementPeriods() *SubscriptionEntitlementPeriodQuery {
...
// QueryPaymentBalanceHolds chains the current query on the "payment_balance_holds" edge.
func (_q *UserQuery) QueryPaymentBalanceHolds() *PaymentBalanceHoldQuery {
...
// WithSubscriptionEntitlementPeriods tells the query-builder to eager-load the nodes that are connected to
// the "subscription_entitlement_periods" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithSubscriptionEntitlementPeriods(opts ...func(*SubscriptionEntitlementPeriodQuery)) *UserQuery {
	query := (&SubscriptionEntitlementPeriodClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithPaymentBalanceHolds tells the query-builder to eager-load the nodes that are connected to
// the "payment_balance_holds" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithPaymentBalanceHolds(opts ...func(*PaymentBalanceHoldQuery)) *UserQuery {
	query := (&PaymentBalanceHoldClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPaymentBalanceHolds = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [17]bool{
			...
			_q.withPaymentBalanceHolds != nil,
	...
	if query := _q.withSubscriptionEntitlementPeriods; query != nil {
		if err := _q.loadSubscriptionEntitlementPeriods(ctx, query, nodes,
	...
	if query := _q.withPaymentBalanceHolds; query != nil {
		if err := _q.loadPaymentBalanceHolds(ctx, query, nodes,
			func(n *User) { n.Edges.PaymentBalanceHolds = []*PaymentBalanceHold{} },
			func(n *User, e *PaymentBalanceHold) {
				n.Edges.PaymentBalanceHolds = append(n.Edges.PaymentBalanceHolds, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadSubscriptionEntitlementPeriods(ctx context.Context, query *SubscriptionEntitlementPeriodQuery, nodes []*User, init func(*User), assign func(*User, *SubscriptionEntitlementPeriod)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptionentitlementperiod.FieldUserID)
	}
...
func (_q *UserQuery) loadPaymentBalanceHolds(ctx context.Context, query *PaymentBalanceHoldQuery, nodes []*User, init func(*User), assign func(*User, *PaymentBalanceHold)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(paymentbalancehold.FieldUserID)
	}
	query.Where(predicate.PaymentBalanceHold(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PaymentBalanceHoldsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
