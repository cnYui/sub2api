...
// QueryBalancePackages chains the current query on the "balance_packages" edge.
func (_q *UserQuery) QueryBalancePackages() *UserBalancePackageQuery {
	query := (&UserBalancePackageClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithBalancePackages tells the query-builder to eager-load the nodes that are connected to
// the "balance_packages" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithBalancePackages(opts ...func(*UserBalancePackageQuery)) *UserQuery {
	query := (&UserBalancePackageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withBalancePackages = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withBalancePackages; query != nil {
...
func (_q *UserQuery) loadBalancePackages(ctx context.Context, query *UserBalancePackageQuery, nodes []*User, init func(*User), assign func(*User, *UserBalancePackage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userbalancepackage.FieldUserID)
	}
