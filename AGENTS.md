...
- 分组倍率以数据库 `groups.rate_multiplier` 为准。截至 2026-09-17（名称为当天统一后的写法）：
  ...
  | 分组名称 | id | 倍率 | 备注 |
  | --- | --- | --- | --- |
  | Claude0.2倍率（日常二） | 73 | 0.2 | 账号 #1167（huoshenai.net） |
  | Claude0.5倍率（日常一） | 5 | 0.5 | 原名 Claude Kiro（0.45），后改名为 `Claude0.5倍率(日常1)`，09-05 由 0.4 调到 0.5。**没有绑定账号**，见第七节 |
  | Claude0.78倍率（日常三） | 71 | 0.78 | 原生；账号 #1166 |
  | Claude1.5倍率（优质） | 4 | 1.5 | 原 Claude Max；账号 #2 |
  | GPT0.15倍率（日常一） | 9 | 0.15 | 账号 #1128 |
  | GPT0.16倍率（日常二） | 13 | 0.16 | 唯一账号 #1132 停调度中，见第七节 |
  | GPT0.16倍率（日常三） | 69 | 0.16 | 账号 #1164 |
  | GPT0.28倍率 | 74 | 0.28 | 账号 #1168（huoshenai.net，与 13/69 同主机、不同 key） |
  | GPT0.35倍率（优质） | 10 | 0.35 | 账号 #1129 |
  | GPT生图1倍率 | 12 | 1.0 | 原名 `Image-2生图`；生图渠道单独隔离，见坑 6 |
  | Grok0.15倍率 | 75 | 0.15 | |
  | Gemini1倍率 | 70 | 1.0 | 原生 |
  | DeepSeek0.4倍率 | 77 | 2.8 | 2026-09-17 新增，账号 1170、渠道 6。只开 `deepseek-v4-flash`、`deepseek-v4.1-flash`，登记价 `$0.30/$1.20/读$0.006`，与火神标准价、DeepSeek 官方高峰价相同 |
  | DeepSeek1倍率 | 8 | 7.0 | 原名 `【国产】DeepSeek`；2026-09-16 由 `4.9` 改为原价 |
  | GLM0.4倍率（无GLM5.3） | 78 | 2.8 | 2026-09-17 新增，账号 1171、渠道 7。只开 `glm-5.2`（火神没有 glm-5.3），登记价 `$1.40/$4.40/读$0.26` |
  ...
  | Kimi0.4倍率 | 79 | 2.8 | 2026-09-17 新增，账号 1172、渠道 8。只开 `kimi-k2.6`、`kimi-k2.7-code`、`kimi-k3`，按 Kimi 官方价登记：k2.6 `$0.95/$4/读$0.16`，k2.7-code `$0.95/$4/读$0.19`，k3 `$3/$15/读$0.30`。上线记录见 `docs/ai/context/20260917-121814-huoshenai-guomo-groups-rollout_CN.md` |
  | Kimi0.7倍率 | 7 | 4.9 | 账号 #5 |
...
- `gpt-6-astra` 两处均已登记：$10/$1/$12.50/$50 每百万 token，>272K 输入转 2x 输入与缓存、1.5x 输出（复用 `openAIGPT54LongContext*` 常量）。别名 `gpt-6`、`gpt6`、带 effort/日期后缀的写法在我方都解析到同一份价格，回归测试见 `billing_service_gpt6_test.go`。**但上游只认精确串 `gpt-6-astra`，别名一律 404**，所以坑 11 的别名少收在这条链路触发不了。
- `gpt-6-astra` 已开放并实测计费正确，当前在 4 个 GPT 分组的白名单里：13、69、74、10（13 的账号停调度中）。**`#1128`（0.15）与 `#1130`（0.1，已下架）未加，也不要照搬着加**——同一 `base_url` 不同 key 的模型清单不同，见坑 24。2026-09-17 复查时，`#1128` 的 `sync-upstream` 清单里已经有 `gpt-6-astra`，但要不要加进分组 9，由管理员决定，加之前先用 `/test` 验证能调通。
...
- **多个账号的白名单里有上游已经没有或调不通的模型**，广场照样展示。以下是 2026-09-17 的核对结果，数据来自 `sync-upstream`（坑 24）和近 7 天的成功请求：

  | 账号 | 分组 | 实际能用的模型 | 白名单里调不通或上游没有的模型 |
  | --- | --- | --- | --- |
  | #5 | 7 | kimi-k3（上游只剩它） | k2.5、k2.6，请求返回 502 |
  | #6 | 8 | deepseek-v4-pro | deepseek-v4-flash，见本节第一条 |
  | #1168 | 74 | gpt-5.6-sol、gpt-6-astra（上游只有这两个） | — |
  | #1128 | 9 | gpt-5.5、sol、terra | gpt-5.4 系列、gpt-5.6、luna，请求返回 502 |
  | #1129 | 10 | gpt-5.5、sol、terra、astra | luna，一天 52 次 502 |
  | #1167 | 73 | — | 上游清单里没有 claude-fable-5 |
  | #2 | 4 | — | 上游清单里只有不带日期的 claude-opus-4-5，没有 claude-opus-4-5-20251101 |
  | #1166 | 71 | — | 白名单里的 Claude 3.x、4、4.1、4.5 系列，上游清单里都没有 |

  - 分组备注已经按实际能用的模型写好。
  - **是否收紧白名单还没定**：上游可能恢复，收紧后还得再加回来。要改的话先 GET 再整体 PUT，见坑 15、20。
  - **白名单改了或上游恢复了，要同步改分组备注。**
- **分组 5 和 13 实际不可用，但仍有用户持有这两个分组的 key**：
  ...
  - **待管理员决定**：补账号、恢复调度，还是下架。恢复后记得把备注改回模型列表。
