...
- 当前 `BILLING_FINAL_MULTIPLIER=19`，**隐藏倍率**，以运行态容器环境变量为准。**2026-09-25 10:25:24（+08）由 17 改为 19**（管理员指示）；此前 2026-09-17 14:42:20（+08）由 18 改为 17。
  ...
  - **`usage_logs` 不记录最终倍率**：核对历史扣费要按 `created_at` 分段（时间均为 +08）：09-17 14:42:20 之前乘 18，到 09-25 10:25:24 之前乘 17，之后乘 19（坑 25）。
  - 记录与回滚见 `docs/ai/context/20260917-155102-final-multiplier-18-to-17_CN.md`、`docs/ai/context/20260925-102825-final-multiplier-17-to-19_CN.md`。
...
- 分组倍率以数据库 `groups.rate_multiplier` 为准。截至 2026-09-17（名称为当天统一后的写法）：
  ...
  | GLM0.6倍率 | 76 | 4.2 | 原名 `GLM模型`，2026-09-16 新增，**2026-09-17 接上账号 1173 后可用**。渠道 5 登记 z.ai 官方 USD 价：glm-5.3 `$1.40/$4.40/读$0.26`、glm-5.3-flash `$0.15/$0.50/读$0.03`，缓存写 0（官方存储限时免费），`restrict_models=true`。上游 `api2.ai-genesis.app` 分组 52（倍率同为 4.2），账号必须强制走 Chat Completions，见坑 31。利润薄：按 ¥29 套餐额度用满、ai-genesis 按 1:1 充值算，收入约为成本的 1.81 倍（= 19 ÷ 10.48；隐藏倍率 17 时 1.62 倍） |
  ...
  | Kimi1倍率 | 7 | 7.0 | 账号 #5（ai-genesis 上游分组 51「月之暗面 Kimi」，上游倍率 4.9）。**2026-09-23 管理员指示由 `Kimi0.7倍率` / 4.9 改为 `Kimi1倍率` / 7.0**，收入/成本从约 1.62 倍升到约 2.32 倍（7×17÷10.48 = ¥11.36 对 4.9×¥1）；09-25 隐藏倍率改 19 后约 2.59 倍（7×19÷10.48 = ¥12.69）。77～79 停用后**这是唯一能用 kimi-k3 的在售分组**，备注里已单独写明；账号白名单 2026-09-23 已收紧为只有 `kimi-k3`（k2.5、k2.6 上游 404）。记录见 `docs/ai/context/20260923-093500-group7-kimi1-rate-7_CN.md` |
  | DeepSeek0.4倍率（日常二） / GLM0.4倍率（日常二） / Kimi0.4倍率（日常二） | 81 / 82 / 83 | 2.8 | 2026-09-23 新增，账号 1175/1176/1177、渠道 10/11/12，各只开一个模型：`deepseek-v4.1-flash` / `glm-5.3-flash` / `kimi-k2.6`，登记价照抄渠道 6/5/8。上游是 `api2.ai-genesis.app` 分组 58「国产模型特惠」（4x）：**三把 key 在同一上游分组、同一钱包，上游都能调全部 5 个模型，隔离全靠我方白名单**；该上游分组的 glm-5.3 恒返回 400，未开放。**利润薄**：kimi-k2.6、glm-5.3-flash 我方登记价 = 上游标准价，收入约为成本 1.27 倍（2.8×19÷10.48 = ¥5.08 对 4×¥1），保本分组倍率约 2.21（隐藏倍率 17 时是 1.14 倍、2.47）；deepseek 上游 08:52 实测 `$0.15/$0.60`、09:04 起变成 `$0.30/$1.20`（疑似分时段价），按后者同样约 1.27 倍；上游缓存读价与我方一致（ds 0.006、kimi 0.16）。上线记录见 `docs/ai/context/20260923-093000-add-daily2-domestic-groups-81-83_CN.md` |
...
- **分组命名与备注规范**（2026-09-17 管理员确定，18 个分组已按此改完，记录见 `docs/ai/context/20260917-125401-group-names-descriptions-unify_CN.md`）：
  ...
  - **备注不写「官方价 X 折 / 原价」**，因为国产分组的这个折扣和用户实付对不上：
    - 在售套餐每 1 元约得 $10.45 余额，扣费时再乘 19，所以广场上每 $1 实际要花约 1.82 元（隐藏倍率为 17 / 18 时约 1.63 / 1.72 元）；
    - 国产官网人民币价约为美元价的 6.8 倍；
    - 两者相乘，国产分组实付 ≈ 官网价 × 分组倍率 × 0.27，比如 2.8 约为 7.5 折，7.0 约为官网价的 1.87 倍（隐藏倍率 17 时是 × 0.24、6.7 折、1.67 倍）；
...
- 余额 `> 0`：不切流量卡；余额不够付这一笔时整笔透支，记为负余额（「最后一次透支」，有意保留）。
- 余额 `<= 0`：下一次请求**不再扣普通余额**，必须使用用户级全渠道流量卡。**余额恰为 0 与欠费同样处理**（2026-09-24 起）：旧逻辑放行 0 余额并把下一笔整笔透支到余额，买了流量卡的用户（505 的 $31.48、674 的 $3.95）第一笔都记成了余额欠款，而只买流量卡的人不会再用套餐到账抵回，等于少收；没买任何东西的新用户也能白拿一次任意大小的请求。例外：简易模式（不计费）和订阅分组（按订阅额度计费）0 余额照常放行，只拦欠费。扣费事务里 0 余额且没有任何流量卡时仍透支到余额（只可能是并发把余额扣到 0），留给下一期套餐抵扣。
...
- **`batch_image.enabled` 默认 `false`，保持关闭**。批量生图结算不经 `applyFinalBillingMultiplier`（`batch_image_settlement.go:138`），开了会**少收 (N−1)/N**（N 为隐藏倍率，当前 19，即约 94.7%），且冻结额同样少 N 倍所以不会报错、静默通过。要开必须先补乘最终倍率。
...
- **火神的 qwen、doubao 分组管理员决定暂不上**（2026-09-17）：火神目前对这两家不收费，官网价已整理进 `20260917-114913` 实测文档。已上的 77～79 与火神逐条对账过：token 数一致，DeepSeek、GLM、k3 的标准费用与火神分毫不差；按最便宜的套餐、额度用满算（1 元 = 10.48 余额美元，扣费再乘隐藏倍率 19），用户实付约为官网人民币价的 7.5 折，火神成本约为官网人民币价的 5.5%，**收入约为成本的 14.0 倍**（= 分组倍率之比 2.8/0.4 = 7，× 「19 ÷ 10.48」与「1 ÷ 1.1」之比 1.99）。隐藏倍率为 17 时这两个数是 6.7 折和 12.5 倍，为 18 时是 7 折和 13.2 倍。
