...
- 将 `13052071067@phone.com` 对应的 Sub2API 用户改为真实邮箱 `milesyang987@gmail.com`。
...
- 目标用户存在：
  - `users.id=3`
  - `users.email=13052071067@phone.com`
  - `users.username=13052071067@phone.com`
  - `role=user`
  - `status=active`
...
- 用户存在一条 `auth_identities` email 身份：
  - `provider_type=email`
  - `provider_key=email`
  - `provider_subject=13052071067@phone.com`
