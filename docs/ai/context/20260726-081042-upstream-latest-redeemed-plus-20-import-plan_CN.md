# 内层 latest redeemed plus 20 账号导入计划

## 背景

- 附件：`C:/Users/yui/Downloads/redeemed_sub2api_20260725230912/sub2api/sub2api.json`
...
- 文件结构：标准 Sub2API 导出，`accounts=20`、`proxies=0`。
- 账号类型：全部 `platform=openai`、`type=oauth`、`plan_type=plus`。
- 文件内 `name`、邮箱、`chatgpt_account_id` 均无重复。
- DB 去重：按 `name`、邮箱与 `chatgpt_account_id` 均无命中。
