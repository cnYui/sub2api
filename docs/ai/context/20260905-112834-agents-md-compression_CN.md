# AGENTS.md 压缩与 CLAUDE.md 软链接改造

<!-- prune:keep -->  <!-- slim 判据来源，被 AGENTS.md 引用，永久保留 -->

- 时间：2026-09-05 11:28（+09）
- 类型：文档结构改造（不涉及生产数据、代码逻辑与运行配置）
- 结果：`AGENTS.md` 由 392 行 / 103,650 字节压缩为 134 行 / 10,969 字节（约 −89%）；`CLAUDE.md` 由 `@AGENTS.md` 导入改为指向 `AGENTS.md` 的符号链接

## 1. 为什么压缩

`AGENTS.md` 是每个会话都会完整载入上下文的文件。它累积了 260 条 `- ` 条目、约 103 KB，其中绝大多数是**一次性运维执行流水**：给某个用户发流量卡、提前刷新某一期周额度、取消某个套餐、重启某次容器、把最终倍率从 16 改成 18 再改回 16。这些条目每条都已经有独立的 `docs/ai/context/*.md` 执行记录，条目本身只是索引。

代价是每次会话都要先读完 103 KB 才能开始干活，而其中真正会影响下一步决策的内容不足两成。压缩的目标不是删历史，是把"每次都要读"的部分收敛到**当前生效的事实、规则和会踩的坑**。
...
| 判据 | 例子 |
| --- | --- |
| 当前仍然生效的**协作约定** | 默认中文；金额以服务端为准；上下文文档只新增不覆盖 |
| 当前的**系统状态**，读错会做错事 | 服务跑在哪台机器、当前最终计费倍率、防火墙与备份怎么配的 |
| 当前生效的**业务规则** | 负余额与流量卡的准入、余额套餐续费周期重置、退款准入与公式 |
| **坑**：踩过一次、下次还会踩、且症状会误导排查方向 | `DOCKER-USER` 第一条规则不能少；JWT secret 实际来自 `config.yaml`；迁移 checksum 保护 |
| **未完成事项** | 外部拨测、单点故障冗余 |
...
| 判据 | 数量级 |
| --- | --- |
| 针对单个用户的一次性执行流水（发放 / 刷新 / 取消 / 退款 / 核查） | 约 80 条 |
| 已被后续变更覆盖的中间状态（最终倍率的历次调整、分组倍率的反复校准） | 约 45 条 |
| 一次性的部署、重启、健康检查、镜像替换记录 | 约 35 条 |
| 已在文中明确标注废弃的规则 | 如 `BILLING_MINIMUM_BALANCE_RESERVE=0.01` 保底阈值 |
| 前端 UI 微调、页面文案调整、教程截图更新 | 约 25 条 |

**没有任何一条被真正删除**——每条移除的条目都指向一个仍然存在的 `docs/ai/context/*.md`，且完整原文归档在本文档第 5 节。

## 3. 三处需要特别说明的取舍

**倍率历史全部移除，只保留当前值和公式。** 最终计费倍率在 8 月经历了 `15 → 18 → 15 → 18 → 20 → 18 → 16 → 17 → 18` 共九次调整，每次都有一条 AGENTS.md 记录。保留这些不能帮助判断"现在是多少"，反而容易读到一半就拿了个过期值。压缩后只写当前值 `18` 和公式，并注明以运行态容器环境变量为准。

**上游定价的反复校准过程移除，只保留最终口径。** Kimi 分组倍率在 8 月 5 日一天内经历 `3.5 → 1.0 → 3.5 → 1.0`，起因是"上游截图里的费用到底是标准费用还是用户价"反复确认了四轮。这个过程本身没有复用价值，但**结论有**，且结论的反面（按时间窗口把上游账单归属到本地用户）是一个已经被明确撤回的错误方法——两者都保留在压缩版的"计费口径"和"负面教训"里。

**保留了三条"我们做错过"的记录。** 这类条目通常最容易在压缩时被当成噪音删掉，但它们恰恰是最不该丢的：上游 Usage 时间窗口对账已撤回（不能用它补扣）、`nc -z` / `wget 401` / 管道到 `head` 三种验证假阳性、Vite `vendor-*` 分包被 Cloudflare 403。

## 4. CLAUDE.md 改为符号链接

改造前 `CLAUDE.md` 内容是单行 `@AGENTS.md`（Claude Code 的导入语法），改造后是指向 `AGENTS.md` 的符号链接：
...
- PowerShell `Get-Item -Force` 报 `LinkType: SymbolicLink`、`Target: AGENTS.md`、`Attributes: Archive, ReparsePoint`——是 **Windows native 符号链接**，不是 MSYS 模拟或副本。本机已启用开发者模式，`ln -s` 可直接创建。
- **`CLAUDE.md` 被 `.gitignore:121` 忽略，且从未被 git 跟踪**（`git ls-files --error-unmatch CLAUDE.md` 报 `did not match any file(s) known to git`）。因此这个软链接**纯粹是本机文件，不会进仓库、不会分发给协作者**——这一点在改造前后完全一致，改造前的 `@AGENTS.md` 单行文件同样是本地的。
- 由此，"其他机器克隆后 symlink 退化成普通文本文件"这个常见的 Windows git symlink 陷阱在这里**不适用**。仓库里唯一被跟踪的是 `AGENTS.md`；新环境需要 `CLAUDE.md` 时自行创建，与本次改造无关。
- `git status` 改造后只显示 `M AGENTS.md` 和新增的本文档，`CLAUDE.md` 因被忽略而完全不出现。

一个附带影响值得知道：较新版本的 Claude Code 会把 `AGENTS.md` 也作为项目指令**独立加载**。改为软链接后 `CLAUDE.md` 的内容即全文，两者可能各载入一次，**上下文层面反而比原先的 `@AGENTS.md` 导入多一份**。压缩后全文仅 11 KB，这个开销可以忽略；但如果要彻底去重，正确做法是**直接删除本地 `CLAUDE.md`**（它是 gitignored 的本地文件，删了不影响仓库），只留 `AGENTS.md`。是否这么做取决于所用 Claude Code 版本是否原生加载 `AGENTS.md`，本次按用户要求保留软链接方案。

## 5. 完整原文归档

以下为压缩前 `AGENTS.md` 的逐字全文（392 行，103,650 字节），供后续检索历史条目使用。

---

# 项目协作约定

- 2026-09-05：在 `DOCKER-USER` 链加结构性兜底，阻断公网主动入站容器。核查确认改造前本就没有暴露（postgres/redis 仅 EXPOSE 未发布，应用只发布到 `127.0.0.1:8080`，DOCKER 链那条 DNAT 的 destination 即 `127.0.0.1`，公网实测 8080/5432/6379 全不可达），但留着一个雷：基础 compose 写的是 `"${BIND_HOST:-0.0.0.0}:..."` 而 `.env` 里 `BIND_HOST=0.0.0.0`，一旦漏掉 `-f docker-compose.vps.yml` 就会真的绕过 UFW 暴露。规则为 `-i eth0 ESTABLISHED,RELATED -j RETURN` + `-i eth0 -j DROP`，**第一条绝不能少**（容器访问上游的返回包也从 eth0 进入，只写 DROP 会掐断全部上游调用）；该机启用了 IPv6，`ip6tables` 已同步配置。变更全程带 `systemd-run --on-active=5min` 自动回滚保险，备份在 `/opt/sub2api/fw-backup-<ts>/`。持久化为 `docker-user-firewall.service`（`PartOf=docker.service`，已实测清空后能完整恢复且脚本幂等）。配套把 `.env` 的 `BIND_HOST` 改为 `127.0.0.1`，至此该风险有三层防护。验证：三域名 200、真实 API 调用 200、容器出站与容器间通信正常、端口仍不可达。详见 `docs/ai/context/20260905-110342-docker-user-firewall-hardening_CN.md`。

- 2026-09-05：公网服务已从笔记本 `sub2api-official-18082` 整体切换到 DediOne 洛杉矶 VPS（`${OPS_VPS_HOST}`），复用同一 tunnel `${OPS_TUNNEL_ID}` 故 DNS 未改动，两次停机窗口合计约 9 分钟。切换前拦下两个致命问题：`BILLING_FINAL_MULTIPLIER=18` 因基础 compose 无 `env_file:` 而未进入容器（会只收应收的 1/18），以及两端 JWT secret 不一致（会使全部用户网页会话失效，笔记本实际生效值来自 `/app/data/config.yaml` 而非 `.env`）。数据以 `DROP/CREATE + pg_restore -j4` 整库替换，恢复后重新施加 `api_key_acl_trust_forwarded_ip=false`；用真实调用验证隐藏倍率仍为 `total_cost × 分组倍率 × 18`。笔记本全部容器与 Cloudflared 服务已停止、watchdog 计划任务已删除，数据卷原样保留可回滚。新增云端看门狗（每 2 分钟，故障注入实测 22.5 秒恢复）、Cloudflare R2 异地备份（桶 `${OPS_R2_BUCKET}`，令牌限 IP `${OPS_VPS_HOST}`，已验证 105.11MB 对象落盘）、开启 `totp_enabled` 与 `step_up_enabled`、硬删除默认管理员 `admin@sub2api.local`、`DATABASE_MAX_OPEN_CONNS` 由 50 降至 25。容量实测：瓶颈是 CPU 不是带宽（真实流量仅占 200Mbps 的 0.3%，iowait 0.0%），每请求约 121ms CPU，上限约 990 请求/分钟或 425 条并发流。**尚未做：外部拨测、单点故障冗余。** 详见 `docs/ai/context/20260905-100322-vps-cutover-hardening-and-capacity-audit_CN.md`。

- 2026-08-31：按要求继续恢复公网服务。检查确认应用、PostgreSQL、Redis、Nginx 均正常，只有 `cloudflared` 进程缺失导致三个公网域名返回 `530`；使用既有凭证和配置恢复 Tunnel `${OPS_TUNNEL_ID}`，进程 ID `37684`，成功注册 4 条连接。本地 `18082`、Nginx `8080` 及 `aaccx.pw`、`www.aaccx.pw`、`api.aaccx.pw` 的 `/health` 均返回 200；未重启应用栈，未修改 DNS、代码、业务数据或数据卷，详见 `docs/ai/context/20260831-130507-public-service-recovery_CN.md`。
...
- 2026-08-30：按要求重新启动公网应用容器 `sub2api-official-18082`。重启前发现 Cloudflare Tunnel 进程已退出，先使用既有凭证恢复 Tunnel `${OPS_TUNNEL_ID}`，再执行 `docker restart`；应用恢复为 `running/healthy`，PostgreSQL、Redis、Nginx 未重启且数据卷未修改。重启后本地 `18082`、Nginx `8080` 及 `aaccx.pw`、`www.aaccx.pw`、`api.aaccx.pw` 的 `/health` 均返回 200，`/usage-guide` 返回 200；Tunnel 成功注册 4 条连接，详见 `docs/ai/context/20260830-202216-public-service-restart_CN.md`。

- 2026-08-30：按要求恢复当前项目公网服务。Docker Desktop Linux 引擎此前未运行，启动后既有 `sub2api-official-18082`、PostgreSQL、Redis 和 `sub2api-public-nginx-local` 容器自动恢复；应用、PostgreSQL、Redis 均 healthy，应用绑定 `127.0.0.1:18082`，Nginx 监听 `127.0.0.1:8080`。使用既有 Cloudflare Tunnel `${OPS_TUNNEL_ID}` 凭证和配置启动 `cloudflared`，4 条 Tunnel 连接注册成功；本地 `18082`、Nginx `8080` 及 `aaccx.pw`、`www.aaccx.pw`、`api.aaccx.pw` 的 `/health` 均返回 200，`/usage-guide` 返回 200。未重建镜像、数据库、Redis、Nginx，未修改数据卷、业务数据或运行配置，详见 `docs/ai/context/20260830-083933-public-service-recovery_CN.md`。
...
- 2026-08-24：退款与商品边界重构收尾：当前商品仅余额套餐和流量卡，普通余额/旧订阅后端不再兼容；流量卡不可退款，仅余额套餐按服务端退款报价退款。成功退款统一写入 `REFUNDED`，移除全额退款、强制退款和余额/订阅扣减分支；历史 Ent/数据库字段仅保留只读查询。退款专项测试、ZPay 适配测试、后端编译和前端类型检查通过；全量前端仍有 4 个既有断言失败及 10 个缺少 `getLiveCapability` mock 的未处理错误。本次未提交、推送或部署，详见 `docs/ai/context/20260824-162517-payment-product-refund-cleanup_CN.md`。
...
- 2026-08-13：恢复 Docker Desktop Linux 引擎后，既有 `sub2api-official-18082`、PostgreSQL、Redis 与本地公网 Nginx 容器自动恢复；按既有配置启动 Cloudflare Tunnel `${OPS_TUNNEL_ID}`。应用仅绑定 `127.0.0.1:18082`，Nginx 转发 `127.0.0.1:8080`；本地和 `aaccx.pw`、`www.aaccx.pw`、`api.aaccx.pw` 的健康检查均为 200。未重建镜像、数据库、Redis、Nginx 或数据卷，详见 `docs/ai/context/20260813-060905-local-start-and-public-tunnel-recovery_CN.md`。
...
- 2026-08-09：`/usage-guide` 的 Codex 接入教程更新为通过 CC Switch 使用 KIMI 分组 API Key 的 10 步图文流程；第 3、4 步按用户指定交换图片顺序，含完整 API Key 的第 6、9 张截图已在入库前遮盖为 `sk-xxxx`。详见 `docs/ai/context/20260809-211336-codex-ccswitch-kimi-guide-update_CN.md`。
...
- 2026-08-04：公网链路为 `aaccx.pw / www.aaccx.pw / api.aaccx.pw -> Cloudflare Tunnel ${OPS_TUNNEL_ID} -> 127.0.0.1:8080 -> sub2api-public-nginx-local -> host.docker.internal:18082`。
...
- 2026-08-11：本地重新构建 `deploy-sub2api:latest` 后，仅替换公网 `sub2api-official-18082` 应用容器；未重启 Docker Desktop 引擎，PostgreSQL、Redis、Nginx、Cloudflare Tunnel 和数据卷均保持不变。应用 healthy，本地、Nginx、三个公网健康端点及 `/usage-guide` 均返回 200。详见 `docs/ai/context/20260811-141246-local-rebuild-public-container-replace_CN.md`。
