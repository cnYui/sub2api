# 内层 latest redeemed plus 10 账号导入计划

## 背景

- 附件：`C:/Users/yui/Downloads/redeemed_sub2api_20260725113032/sub2api/sub2api.json`
...
- 文件结构：标准 Sub2API 导出，`accounts=10`、`proxies=0`。
- 账号类型：全部 `platform=openai`、`type=oauth`、`plan_type=plus`。
- 文件内重复：`name=0`，`chatgpt_account_id=0`。
