...
test('buildRuntimeSql writes an alipay-only EasyPay provider using ZPay hosted cashier', () => {
  const sql = buildRuntimeSql(env, { refundEnabled: true, allowUserRefund: true })
  ...
  assert.match(sql, /10, '', true, 'popup', true/)
  assert.doesNotMatch(sql, /payment_mode = 'qrcode'/)
  assert.match(sql, /refund_enabled = true/)
  assert.match(sql, /allow_user_refund = true/)
